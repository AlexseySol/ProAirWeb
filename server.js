...
app.use(
  helmet({
    contentSecurityPolicy: {
      directives: {
        defaultSrc: ["'self'", "https://secure.wayforpay.com"],
        scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://secure.wayforpay.com"],
        connectSrc: ["'self'", "https://secure.wayforpay.com"],
        frameSrc: ["'self'", "https://secure.wayforpay.com"],
        imgSrc: ["'self'", "data:", "https:"],
...
app.post('/api/initialize-payment', async (req, res) => {
  try {
    ...
    const paymentData = {
      ...
      productCount: [productCount],
      clientFirstName: name,
      clientLastName: position || "Н/Д",
      clientEmail: email,
      clientPhone: phone,
      language: "UA",
      returnUrl: `${process.env.REACT_APP_API_URL || 'https://proair.website'}/api/payment-result`,
      serviceUrl: `${process.env.REACT_APP_API_URL || 'https://proair.website'}/api/wayforpay-callback`,
    };
